    ...
    <>
      <button className='mt-1 transition duration-150 ease-in-out' onClick={handleOpen}>
        ...
        <div className='flex justify-start border-r-2 '>
          <button className='text-[13px] font-normal tracking-wide flex items-center gap-x-1 bg-[#f6f6f6] text-[#1f2228] py-1 -mb-10 mt-2  px-2 rounded-lg'>New Group <BsPlusLg /></button>
        </div>
      </button>
      <Modal
        open={open}
      ...
      >
        <Box sx={style} className='rounded-lg'>
          <h5 className='text-[18px] text-[#111b21] font-medium text-center'>Create Group</h5>
          ...
          <form onSubmit={(e) => e.preventDefault()} className='flex flex-col gap-y-3 mt-3'>
            ...
            <div className='flex -mt-2'>

              {
                selectedUser?.map((e) => {
                  return (
                    <button key={e} onClick={() => deleteSelected(e)} className='flex items-center gap-x-1 bg-green-100 text-green-800 text-xs font-medium mr-2 px-2.5 py-0.5 dark:bg-gray-700 dark:text-green-400 border border-green-400 rounded-lg'>
                      <span >{e.name}</span>
                      <RxCross2 />
                    </button>
                  )
            ...
            <div className='flex justify-end mt-3 '>
              <button onClick={handleSubmit} className='bg-[#0086ea] text-[#fff] text-[15px] font-medium px-2 py-1 tracking-wide rounded-lg' type='submit'>Create</button>
            </div>
          </form>
