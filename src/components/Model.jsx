    ...
    <>
      ...
      </button>
      {
        activeChat?.isGroup ?
              ...
              <div>
                ...
                <div>
                  <form className='mt-5 flex flex-col gap-y-3' onSubmit={(e) => e.preventDefault()}>
                    <input onChange={(e) => setName(e.target.value)} value={name} className="border-[#c4ccd5] border-[1px] text-[13.5px] py-[4px] px-2 w-[100%] rounded-lg" type="text" name="chatName" placeholder="Group Name" required />
                    <input onChange={(e) => setSearch(e.target.value)} className="border-[#c4ccd5] border-[1px] text-[13.5px] py-[4px] px-2 w-[100%] rounded-lg" type="text" name="users" placeholder="add users" />
                  </form>
                  ...
                  <div className='flex justify-end gap-x-3 mt-3'>
                    <button onClick={updateBtn} className='bg-[#0086ea] transition hover:bg-[#00A1C9]  px-4 py-1 text-[10.6px] tracking-wide text-[#fff] rounded-lg'>Update</button>
                    <button onClick={() => leaveGroup()} className='bg-[#880808] hover:bg-[#A52A2A] transition delay-150 px-4 py-1 text-[10.6px] tracking-wide text-[#fff] rounded-lg'>Leave</button>
          ...
          >
            <Box sx={style} className='rounded-lg'>
              <div className='w-[25rem] h-[250px] flex flex-col items-center justify-center -mt-4 gap-5'>
                ...
                <div className='flex flex-col items-center'>
                  <h5 className='text-[13px] flex flex-col justify-center'>{!activeChat?.isGroup && activeChat?.users[0]?._id === activeUser.id ? activeChat?.users[1]?.bio : activeChat?.users[0]?.bio}</h5>
                </div>
              </div>
            </Box>
          </Modal>
